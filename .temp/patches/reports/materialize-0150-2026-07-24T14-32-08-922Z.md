# Patch Dry Run materialize-0150-2026-07-24T14-32-08-922Z
...
| 147 | `f0cc012dbc26` | failed | check | conflict | feat(provider): 添加版本覆盖选项，支持自定义提供者的客户端版本 |

## Failure Output

### #147 f0cc012dbc264dd89529abd629d6c2dcc5544a17
...
error: while searching for:
        for (const [providerID, provider] of configProviders) {
          ...
          const inheritedModels = existing
            ? Object.fromEntries(Object.entries(existing.models).map(([mid, model]) => [mid, { ...model, providerID: ProviderV2.ID.make(providerID), api: { ...model.api, npm: provider.npm ?? model.api.npm, // 继承克隆：子 provider 覆盖 npm/api 后仍走默认映射

error: patch failed: packages/opencode/src/provider/provider.ts:1484
