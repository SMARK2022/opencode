...
| `--color auto` + TTY | `useColor()` | 由 TTY 与 `NO_COLOR` 决定 | 所有 Stats renderer | `charts.ts` color seam | observed / contracted |
| `NO_COLOR` | process environment | auto 模式关闭颜色 | 所有 Stats renderer | `useColor()` | observed / contracted |
...
| INV-FG-02 | Grid、axis、text、series 使用同一共享 semantic palette；页面层不得自行复制或覆盖颜色角色。R3 固定 role-to-ANSI contract：`title=95m`、`subtitle=96m`、`blue=94m`、`cyan=96m`、`green=92m`、`yellow=93m`、`orange=33m`、`purple=95m`、`pink=35m`、`red=91m`、`axis/white=39m`、`muted/grid=2;39m`。只有 `muted/grid` 可以携带 dim。 | `render.ts` 只 alias `charts.ts.palette`；`renderRoundedLineChart()`、tables、stacks 共用 `paint`；`cli/ui.ts` 已使用 bright ANSI semantic roles。 | 部分由 colored breakdown/status 测试覆盖；R3 新增 exact public palette contract 和全入口矩阵。 |
...
3. 将 foreground role mapping 继续集中在 `charts.ts`，采用已经存在于普通 CLI 和 OpenCode system theme 语义中的 ANSI 槽位。R3 的唯一目标表如下；这些是 public `--color always` 输出契约，不是实现者可自由选择的视觉建议：
...
4. 调整 palette 时只修改 role-to-foreground code，不修改 `ChartColor` public union、`seriesColor()` 顺序、render page 调用点或无色分支。R3 的可验收条件是：每个 role 的实际 escape code 与上表一致；数据线不处于 dim；标题/辅助层级有稳定的 cyan/magenta 主题色；系列仍可区分。
...
| Theme hierarchy must be shared | `palette` + `renderContext()` | Implement the exact R3 role table in `charts.ts`; keep `render.ts` alias | Minimal chart role-contract test with independent literal ANSI expectations |
...
| Exact R3 semantic ANSI role table | `INV-FG-02` | `cli/ui.ts`, `generateSystem()` and current palette history | Current palette uses several normal slots and default title/subtitle roles; it cannot satisfy the reported theme hierarchy contract without an explicit mapping. |
...
| `packages/opencode/test/cli/stats-render-width.test.ts` | modify | Add behavior-level ANSI state regression, exact shared palette contract, and complete colored renderer/option matrix; update only assertions whose public role mapping intentionally changes. | +55 to +110 / -0 to -18 |
| `packages/opencode/test/cli/stats-command.test.ts` | add | Invoke the real Stats CLI with temporary database state and argument arrays; verify aliases/root shortcuts and non-TTY/PTY interactive view dispatch without adding a production test seam. | +90 to +180 / 0 |
...
| 3 | Render the exact R3 palette contract through a minimal public chart/text output; assert each changed role emits its literal ANSI code and only muted/grid may set dim. | Current `title=39m`, `blue=34m`, `green=32m` and other roles do not match the R3 contract, so this test is red before implementation. | Replace only the shared palette table with the R3 mapping. | Makes the theme hierarchy deterministic instead of leaving visual choice to the implementer. |
...
| 5 | Render `emptyReport()` with `color=always` and `never` through Dashboard, all seven Breakdown dimensions, Timeline, Sessions and the separately implemented Insights empty branch; assert R3 roles, only `49m` background reset, strip equivalence and fixed width. | Existing empty-page matrix is `color=never` only, so current tests cannot detect a foreground/background regression in the early-return paths. | Reuse the shared palette without changing either empty renderer. | Protects new installation and no-match filters across all page entrypoints. |
| 6 | Spawn the actual Stats CLI against a temporary empty database/cwd using argument arrays for every canonical command, alias and root shortcut; assert process success, expected page identity, R3 title role, no concrete background, and decolored equivalence within each alias class. | Isolated builders cannot prove which handler yargs dispatches or whether root shortcuts propagate `color`. | Add test-only subprocess coverage; do not change `stats.ts`. | Protects all public command routes through their owning CLI interface. |
| 7 | Spawn `stats --interactive --color always` in a PTY, wait for the rendered page identity, send `n` to observe Dashboard -> Breakdown -> Timeline -> Sessions -> Insights, then send `q` and assert cursor/raw cleanup on exit. | Direct renderer tests cannot observe the private callback table, color propagation, key dispatch or finalizer. | Add a deterministic PTY test that waits on output events rather than wall-clock sleeps. | Protects the actual five-view interactive success path and cleanup. |
...
| Dashboard/root aliases | `stats`, `dashboard`, `dash`, `overview`, `summary`, plus root `--models`/`--tools` shortcuts | same Dashboard or Breakdown renderer, same R3 role contract; no alternate palette. |
...
- Create a fixture directory through existing `tmpdir({ git: true })`; point
  `OPENCODE_DB` at an absolute database path under that fixture and set
  `OPENCODE_PURE=1` so the test never reads the user's database or plugins.
...
- Execute every alias class listed in §16.1. Empty output is sufficient for
  dispatch identity because each renderer emits its page title before the
  empty-state message; the direct renderer matrix separately covers populated
  color consumers.
...
- The TTY case must use the existing Bun PTY adapter, accumulate data events,
  wait for each page identity before sending the next `n`, and send `q` only
  after Insights is observed. Use an event/deferred timeout, never a fixed
  sleep. The test must always kill/dispose the PTY on failure and assert the
  final cursor reset sequence.
...
| --- | ---: | --- |
| Effective changed code lines `E` | 170-325 | Includes executable production and behavior-test lines in the three planned implementation files; excludes imports, formatting, this proposal, and existing unchanged comments. |
| Required Chinese explanatory comments `C` | 26-49 minimum | `max(1, ceil(E * 0.15))`; comments distributed near transition reset, each role table boundary, no-background boundary, option matrix intent, command harness isolation and PTY cleanup. |
| Planned qualifying comment locations | 32-58 | Explain why reset precedes color, why background remains separate, why ANSI semantic roles are used instead of renderer probing, why each option class is tested, and why subprocess/PTY tests use isolated state and output signals. |
...
| `bunx oxlint src/cli/cmd/stats/charts.ts test/cli/stats-render-width.test.ts test/cli/stats-command.test.ts` | `packages/opencode` | Scoped lint for the three changed files; use the repository's installed oxlint version. |
...
| 140-column PTY/manual output for Dashboard, Timeline, Model, Provider, Tool, Sessions and Insights | repository root / real macOS terminal | Confirms the corrected foreground remains visible in the reported dark profile without restoring background. |
...
| --- | ---: | --- |
| Files added | 1 | A dedicated actual-command behavior test; no runtime file is added. |
...
| Test lines | +165 to +300 / -0 to -18 | Public ANSI-state test, exact role contract, populated/empty option matrix, actual CLI aliases and PTY interactive cycle. |
...
If implementation exceeds three changed implementation files or introduces a new renderer,
...
- Bright versus normal ANSI slots can change perceived hierarchy across terminal profiles; the fixed R3 role mapping must be checked in both the existing dark profile and a light-profile synthetic/PTY case.
- The current worktree contains unrelated staged/unstaged changes, including `docs/prompts.md`; future implementation must isolate only the three planned Stats files.
...
None for the current R3 implementation scope. The role table in §10.2 is the
...
exact user-authorized behavior rollback. It is intentionally excluded from R3
...
| pending | R3 | pending | pending | pending | pending | pending |
...
Not applicable. This revision explicitly forbids implementation and contains
no production, test, configuration, migration or generated-file changes.

### Actual Files and Diff

Not applicable; future implementation is limited to the file-level plan in §15.

### Red-Green Test Evidence

Not applicable; the read-only harness is recorded in §4.1. The formal behavior
test must be written first during an approved implementation phase.

### Verification Commands and Results

Not applicable for implementation. Existing prior Stats checks remain baseline
evidence only; this focused plan has not changed code and has not rerun a full
test suite.

### Original Feedback-Loop Result

The minimized read-only harness is red-capable: it observes a grid dim start
followed by a series foreground without an intensity reset. It is not yet a
repository test because the current user forbids implementation changes.
...
| --- | ---: | --- |
| Effective changed code lines `E` | 0 | No production or test code changed in this revision. |
| Qualifying Chinese comment lines `C` | 0 | Proposal text is not implementation code. |
| Ratio `C / E` | N/A | `E = 0`. |
| Required minimum `C` | 0 | `if E = 0: C = 0`. |

### Remaining Unverified Items

- The R3 role table is explicit, but its visual result has not yet been observed on the user's macOS terminal because implementation is not authorized.
- The formal red test has not been added or run because implementation is not authorized.
- No real PTY capture was changed or regenerated in this planning-only revision.
...
The task remains `audit-required` and implementation is prohibited until the
plan receives independent full-scope approval.
