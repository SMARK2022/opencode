...
| Recommended CLI | `opencode 1.15.13-smark` |
...
Use `sdks/vscode/package.json` as the canonical version source for local packaging and push-triggered builds. The repository extension version is ordinary semver (`1.15.11`) so Marketplace tooling accepts it; the recommended CLI is `1.15.13-smark`, but the two packages remain independently versioned.
