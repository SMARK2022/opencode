...
Day la dang day du: `bash -s --` bao `bash` doc installer tu stdin va truyen `--version 1.15.12-smark` lam doi so installer. Phien ban co the la `1.15.12-smark` hoac dang release tag `v1.15.12-smark`.
...
De dung dich vu ngon ngu cua VS Code hoac Notebook tools, hay cai [SMARK2022.opencode-ide-bridge](https://marketplace.visualstudio.com/items?itemName=SMARK2022.opencode-ide-bridge). Phien ban extension trong repository la `1.15.10`, va khuyen dung SMARK CLI `1.15.12-smark`; hai phien ban doc lap. Extension tao local authenticated bridge cho dich vu ngon ngu va VS Code/Jupyter Notebook. Khi khong ket noi, CLI van dung LSP tich hop, nhung khong co VS Code-backed operations va Notebook tools.
