...
Tam biçim budur: `bash -s --`, `bash`'e installer'ı stdin'den okumasını ve `--version 1.15.11-smark` değerini installer argümanları olarak geçirmesini söyler. Sürüm `1.15.11-smark` veya release tag biçimi olan `v1.15.11-smark` olabilir.
...
VS Code dil servislerini veya Notebook tools'u kullanmak için [SMARK2022.opencode-ide-bridge](https://marketplace.visualstudio.com/items?itemName=SMARK2022.opencode-ide-bridge) kurun. Repository extension sürümü `1.15.10`, önerilen SMARK CLI sürümü `1.15.11-smark`tır; sürümler bağımsızdır. Extension, dil servisleri ve VS Code/Jupyter Notebook için yerel authenticated bridge oluşturur. Bağlantı olmadığında CLI dahili LSP'yi kullanmaya devam eder, ancak VS Code-backed işlemler ve Notebook tools kullanılamaz.
