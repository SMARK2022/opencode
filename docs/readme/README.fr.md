...
> **À propos de cette branche** : il s'agit de la branche améliorée `dev-smark` d'OpenCode (version actuelle `1.15.14`, tag de release CLI `v1.15.14-smark`). Elle est basée sur la branche amont `dev` et se concentre sur l'interaction TUI, la gestion des sessions, les statistiques de tokens, la compatibilité Windows/PowerShell, les services de langage et l'intégration Notebook de VS Code, la prise en charge des proxys réseau et l'expérience d'installation.
...
C'est la forme complète : `bash -s --` indique à `bash` de lire l'installateur depuis stdin et de transmettre `--version 1.15.14-smark` comme arguments de l'installateur. La version peut être `1.15.14-smark` ou la forme tag de release `v1.15.14-smark`.
