...
> **O tej gałęzi**: To ulepszona gałąź OpenCode `dev-smark` (bieżąca wersja `1.15.14`, tag wydania CLI `v1.15.14-smark`). Jest oparta na upstream `dev` i koncentruje się na interakcji TUI, zarządzaniu sesjami, statystykach tokenów, zgodności z Windows/PowerShell, usługach językowych i integracji Notebook w VS Code, obsłudze proxy sieciowego i doświadczeniu instalacji.
...
To jest pełna forma: `bash -s --` mówi `bash`, aby odczytał instalator ze stdin i przekazał `--version 1.15.14-smark` jako argumenty instalatora. Wersją może być `1.15.14-smark` albo forma tagu wydania `v1.15.14-smark`.
...
| VS Code | Agenci CLI nie mogą bezpośrednio używać usług językowych edytora ani niezawodnie obsługiwać Notebooków | Diagnostyka, nawigacja i symbole przez VS Code, edycja i uruchamianie komórek, zarządzanie kernelem |
...
| `vscode_notebook_env` | Sprawdza kernel/runtime, wybiera, restartuje lub zatrzymuje kernel oraz tworzy lub zapisuje notebook na wyraźne żądanie |
