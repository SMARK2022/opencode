...
এটি সম্পূর্ণ form: `bash -s --` `bash`-কে stdin থেকে installer পড়তে এবং `--version 1.15.13-smark` installer arguments হিসেবে পাঠাতে বলে। version `1.15.13-smark` হতে পারে অথবা release tag form `v1.15.13-smark` হতে পারে।
...
VS Code language services বা Notebook tools ব্যবহার করতে [SMARK2022.opencode-ide-bridge](https://marketplace.visualstudio.com/items?itemName=SMARK2022.opencode-ide-bridge) install করুন। repository extension version `1.15.10`, আর SMARK CLI `1.15.13-smark` recommended; দুটির version স্বাধীন। Extension language services ও VS Code/Jupyter Notebook-এর জন্য local authenticated bridge তৈরি করে। Bridge connected না থাকলে CLI built-in LSP ব্যবহার করে, কিন্তু VS Code-backed operations বা Notebook tools পাওয়া যায় না।
