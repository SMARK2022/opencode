...
Ovo je potpuni oblik: `bash -s --` kaže komandi `bash` da čita installer iz stdin i proslijedi `--version 1.15.12-smark` kao argumente installera. Verzija može biti `1.15.12-smark` ili oblik release taga `v1.15.12-smark`.
...
Za korištenje VS Code jezičkih servisa ili Notebook alata instalirajte [SMARK2022.opencode-ide-bridge](https://marketplace.visualstudio.com/items?itemName=SMARK2022.opencode-ide-bridge). Verzija ekstenzije u repozitoriju je `1.15.10`, a preporučeni SMARK CLI je `1.15.12-smark`; verzije su nezavisne. Ekstenzija kreira lokalni autentificirani bridge za jezičke servise i VS Code/Jupyter Notebook. Bez veze CLI zadržava ugrađeni LSP, ali VS Code operacije i Notebook alati nisu dostupni.
