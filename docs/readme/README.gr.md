...
> **Σχετικά με αυτόν τον κλάδο**: Αυτός είναι ο ενισχυμένος κλάδος `dev-smark` του OpenCode (τρέχουσα έκδοση `1.15.11`, CLI release tag `v1.15.11-smark`). Βασίζεται στον upstream κλάδο `dev` και εστιάζει στην αλληλεπίδραση TUI, στη διαχείριση συνεδριών, στα στατιστικά token, στη συμβατότητα Windows/PowerShell, στις υπηρεσίες γλώσσας και στην ενσωμάτωση Notebook του VS Code, στην υποστήριξη network proxy και στην εμπειρία εγκατάστασης.
...
Αυτή είναι η πλήρης μορφή: το `bash -s --` λέει στο `bash` να διαβάσει τον installer από stdin και να περάσει το `--version 1.15.11-smark` ως ορίσματα installer. Η έκδοση μπορεί να είναι `1.15.11-smark` ή η μορφή release tag `v1.15.11-smark`.
