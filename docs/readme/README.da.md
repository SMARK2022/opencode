...
> **Om denne branch**: Dette er OpenCode `dev-smark` enhanced branch (aktuel version `1.15.14`, CLI release tag `v1.15.14-smark`). Den bygger på upstream `dev` branch og fokuserer på forbedringer af TUI-interaktion, sessionshåndtering, Token-statistik, Windows/PowerShell-kompatibilitet, VS Code-sprogtjenester og Notebook-integration, netværksproxy og installationsoplevelse.
...
Dette er den komplette form: `bash -s --` får `bash` til at læse installer fra stdin og sende `--version 1.15.14-smark` videre som installer-argument. Versionsargumentet kan være `1.15.14-smark` eller release tag-formen `v1.15.14-smark`.
...
| VS Code | CLI-agenter kan ikke direkte genbruge editorens sprogtjenester eller håndtere Notebooks pålideligt | VS Code-baseret diagnostik, navigation og symboler, celleredigering og kørsel, kernelstyring |
...
| `vscode_notebook_env` | Viser kernel/runtime, vælger, genstarter eller stopper kernel og opretter eller gemmer notebook efter udtrykkeligt ønske |
