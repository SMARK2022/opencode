...
> **حول هذا الفرع**: هذا هو فرع OpenCode المحسن `dev-smark`، الإصدار الحالي `1.15.13` ووسم إصدار CLI هو `v1.15.13-smark`. يعتمد على فرع upstream `dev` ويركز على تحسين تفاعل TUI، وإدارة الجلسات، وإحصاءات token، وتوافق Windows/PowerShell، وتكامل خدمات اللغة وNotebook في VS Code، ووكيل الشبكة، وتجربة التثبيت.
...
هذا هو الشكل الكامل: تعني `bash -s --` أن `bash` يقرأ installer من stdin ويمرر `--version 1.15.13-smark` كوسائط للـ installer. يمكن كتابة الإصدار كـ `1.15.13-smark` أو بصيغة release tag وهي `v1.15.13-smark`.
...
لاستخدام خدمات اللغة في VS Code أو أدوات Notebook، ثبت إضافة [SMARK2022.opencode-ide-bridge](https://marketplace.visualstudio.com/items?itemName=SMARK2022.opencode-ide-bridge). إصدار الإضافة في المستودع هو `1.15.10`، ويوصى باستخدام SMARK CLI `1.15.13-smark`؛ ولهما إصدارات مستقلة. تنشئ الإضافة bridge محليا موثقا لخدمات اللغة وVS Code/Jupyter Notebook. بدون اتصالها يستمر CLI باستخدام LSP المدمج، لكن خدمات VS Code وأدوات Notebook لا تتوفر.
