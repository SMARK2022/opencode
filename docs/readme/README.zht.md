...
> **關於本分支**：這是 OpenCode 的 `dev-smark` 增強分支（目前版本 `1.15.13`，CLI release tag 為 `v1.15.13-smark`）。它基於上游 `dev` 分支，重點增強 TUI 互動、會話管理、Token 統計、Windows/PowerShell 相容性、VS Code 語言服務與 Notebook 整合、網路代理及安裝體驗。
...
這是完整寫法：`bash -s --` 表示讓 `bash` 從 stdin 讀取 installer，並把後面的 `--version 1.15.13-smark` 作為 installer 參數傳入。版本參數可以寫 `1.15.13-smark`，也可以寫 release tag 形式的 `v1.15.13-smark`。
...
需要使用 VS Code 語言服務或 Notebook 工具時，請安裝擴充套件 [SMARK2022.opencode-ide-bridge](https://marketplace.visualstudio.com/items?itemName=SMARK2022.opencode-ide-bridge)。倉庫內擴充套件版本為 `1.15.10`，建議配合 SMARK CLI `1.15.13-smark`；兩者獨立進行版本管理。擴充套件為語言服務與 VS Code/Jupyter Notebook 建立本地鑑權 bridge；未連線時 CLI 仍使用內建 LSP，但無法使用 VS Code-backed 操作與 Notebook 工具。
