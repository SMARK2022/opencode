...
> **О ветке**: это расширенная ветка OpenCode `dev-smark` (текущая версия `1.15.14`, CLI release tag `v1.15.14-smark`). Она основана на upstream `dev` и сосредоточена на TUI-взаимодействии, управлении сессиями, статистике token, совместимости с Windows/PowerShell, языковых сервисах и интеграции Notebook в VS Code, поддержке сетевых proxy и опыте установки.
...
Это полная форма: `bash -s --` говорит `bash` читать installer из stdin и передать `--version 1.15.14-smark` как аргументы installer. Версия может быть `1.15.14-smark` или в форме release tag `v1.15.14-smark`.
...
| VS Code | CLI agents не могут напрямую использовать языковые сервисы редактора или надежно работать с Notebook | Диагностика, навигация и символы через VS Code, редактирование и запуск cells, управление kernel |
...
| `vscode_notebook_env` | Inspect kernel/runtime, select/restart/stop kernel и create/save notebook по явному запросу user |
