...
> **О ветке**: это расширенная ветка OpenCode `dev-smark` (текущая версия `1.15.11`, CLI release tag `v1.15.11-smark`). Она основана на upstream `dev` и сосредоточена на TUI-взаимодействии, управлении сессиями, статистике token, совместимости с Windows/PowerShell, языковых сервисах и интеграции Notebook в VS Code, поддержке сетевых proxy и опыте установки.
...
Это полная форма: `bash -s --` говорит `bash` читать installer из stdin и передать `--version 1.15.11-smark` как аргументы installer. Версия может быть `1.15.11-smark` или в форме release tag `v1.15.11-smark`.
