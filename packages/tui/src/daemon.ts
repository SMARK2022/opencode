...
import { ServerLock } from "./server-lock"
...
function hasCliOption(name: string, argv = process.argv) {
  return argv.some((arg) => arg === name || arg.startsWith(`${name}=`))
}

function hasCliBooleanOption(name: string, argv = process.argv) {
...
function resolveNetworkOptionsNoConfig(args: NetworkOptions) {
  return {
    hostname: args.hostname,
    port: args.port,
    mdns: args.mdns,
    mdnsDomain: args["mdns-domain"],
...
function serverAuthHeaders() {
  ...
  return { Authorization: `Basic ${Buffer.from(`${Flag.OPENCODE_SERVER_USERNAME ?? "opencode"}:${password}`).toString("base64")}` }
}
...
async function target() {
  ...
  if (await Bun.file(fileURLToPath(dist)).exists()) return fileURLToPath(dist)
...
async function existingOwnerUrl(external: boolean) {
  ...
  const responsive = await ServerLock.ping(lock.port, { headers: serverAuthHeaders() })
  return {
  ...
  try {
    ...
    while (Date.now() < deadline) {
      ...
      if (lock && ServerLock.alive(lock.pid)) {
        ...
        } else if (await ServerLock.ping(lock.port, { headers: serverAuthHeaders() })) {
          return internalUrl(lock.port)
        }
      }
