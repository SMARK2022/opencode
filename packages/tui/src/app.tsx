...
import { DialogTool } from "./component/dialog-tool"
  ...
  const result = yield* Effect.scoped(
    Effect.gen(function* () {
      ...
      yield* Effect.sync(() => {
        ...
        if (shouldForceUnicodeWidth) process.env.OPENTUI_FORCE_UNICODE = "1"
      })
                                    ...
                                    <TuiConfigProvider config={input.config}>
                                      <PluginRuntimeProvider value={pluginRuntime}>
                                        <SDKProvider
                                          ...
                                          testTransport={
                                            input.fetch || input.events
                                              ? { fetch: input.fetch, events: input.events }
                                              : undefined
                                          }
                                        >
                                          <ProjectProvider>
...
function App(props: { onSnapshot?: () => Promise<string[]>; pluginHost: TuiPluginHost }) {
  ...
  // [local-smark] bash compression toggle state
  ...
  const appCommands = createMemo(() =>
    [
      ...
      // [local-smark] bash compression toggle command
      {
        name: "app.toggle.bash_compression",
      ...
      // [local-smark] tool selection dialog command
      {
        name: "tool.list",
        title: "Toggle tools",
        category: "Session",
