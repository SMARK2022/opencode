...
function formatSkills(skills: ReturnType<typeof skillInfo>) {
  return skills.map((skill) => `- ${skill.name}: ${skill.description || "No description."}`).join("\n")
}
...
const EDIT_TOOL_NAMES = new Set<string>([TOOL_IDS.edit, TOOL_IDS.write, TOOL_IDS.applyPatch])
...
function disabledTools(names: string[], ruleset: readonly PermissionRuleLike[] = []) {
  return new Set(
    names.filter((name) => {
      const permission = EDIT_TOOL_NAMES.has(name) ? TOOL_IDS.edit : name
...
const STATIC_TOOL_DEFINITIONS: ToolDefinitionTemplate[] = [
  { name: TOOL_IDS.invalid, description: "Do not use." },
  { name: TOOL_IDS.question, description: "Ask the user a structured question and wait for an answer." },
  { name: TOOL_IDS.shell, description: "Run terminal commands for build, test, git inspection, package managers, and system operations." },
  { name: TOOL_IDS.read, description: "Read files from the workspace with line-numbered output." },
  { name: TOOL_IDS.glob, description: "Find files by glob pattern." },
  { name: TOOL_IDS.grep, description: "Search file contents with regular expressions." },
  { name: TOOL_IDS.edit, description: "Edit existing files." },
  { name: TOOL_IDS.write, description: "Create or replace files." },
  { name: TOOL_IDS.task, description: "Launch a subagent for independent work." },
  { name: TOOL_IDS.webfetch, description: "Fetch and inspect web content." },
  { name: TOOL_IDS.todowrite, description: "Create and update a structured task list." },
  { name: TOOL_IDS.websearch, description: "Search the web." },
  { name: TOOL_IDS.skill, description: "Load a specialized skill." },
  { name: TOOL_IDS.applyPatch, description: "Apply a patch to files." },
  { name: TOOL_IDS.lsp, description: "Inspect language server information." },
  { name: TOOL_IDS.planExit, description: "Exit planning mode." },
]

function currentShellName() {
  return path.basename(process.env.SHELL ?? process.env.ComSpec ?? (process.platform === "win32" ? "powershell" : "bash")).toLowerCase().replace(/\.exe$/, "")
}

function resolveTemplateVars(text: string, paths: ContextUsagePaths): string {
  if (!text.includes("${")) return text

  const shellName = currentShellName()
...
function baseToolDefinitions(input: ComputeContextDataInput, modelInfo: ReturnType<typeof currentModel>, lastUser: Message | undefined) {
  ...
  return STATIC_TOOL_DEFINITIONS
    .filter((item) => {
      if (item.name === TOOL_IDS.question) {
        return ["app", "cli", "desktop"].includes(Flag.OPENCODE_CLIENT) || Flag.OPENCODE_ENABLE_QUESTION_TOOL
      }
      if (item.name === TOOL_IDS.lsp) return Flag.OPENCODE_EXPERIMENTAL_LSP_TOOL
      if (item.name === TOOL_IDS.planExit) return Flag.OPENCODE_EXPERIMENTAL_PLAN_MODE && Flag.OPENCODE_CLIENT === "cli"
      if (item.name === TOOL_IDS.websearch) {
        return modelInfo.providerID === "opencode" || Flag.OPENCODE_ENABLE_EXA
      }
      if (item.name === TOOL_IDS.applyPatch) return usePatch
      if (item.name === TOOL_IDS.edit || item.name === TOOL_IDS.write) return !usePatch
      return true
    })
...
function activeToolNames(definitions: Array<{ name: string; text: string }>, lastUser: Message | undefined) {
  ...
  if (lastUser?.role === "user" && lastUser.tools) {
    for (const [name, enabled] of Object.entries(lastUser.tools)) {
      ...
      disabled.add(name)
      if (name === "edit") {
        disabled.add(TOOL_IDS.edit)
        disabled.add(TOOL_IDS.write)
        disabled.add(TOOL_IDS.applyPatch)
      }
    }
  }
...
function dynamicToolText(name: string, input: ComputeContextDataInput, skills: Array<{ name: string; description: string; path: string }>, lastUser: Message | undefined) {
  if (name === TOOL_IDS.task) {
    ...
    const description = (input.agents ?? [])
      ...
      .filter((agent) => !current || evaluateRule(TOOL_IDS.task, agent.name, current.permission as PermissionRuleLike[])?.action !== "deny")
      .toSorted((a, b) => a.name.localeCompare(b.name))
      .map((agent) => `- ${agent.name}: ${agent.description ?? "This subagent should only be called manually by the user."}`)
      .join("\n")
    return ["Available agent types and the tools they have access to:", description].join("\n")
  }
  if (name === TOOL_IDS.skill) return skills.length === 0 ? "No skills are currently available." : formatSkills(skillInfo(skills))
  return ""
}
...
function windowTokens(input: ComputeContextDataInput, model: Provider["models"][string] | undefined) {
  ...
  if (!context) return 0
  const providerReserve = model.limit.input ? Math.max(0, context - model.limit.input) : 0
  const reserved = input.config?.compaction?.reserved ?? Math.max(5_000, 20_000 - providerReserve)
  return model.limit.input ? Math.max(0, model.limit.input - reserved) : Math.max(0, context - reserved)
}
