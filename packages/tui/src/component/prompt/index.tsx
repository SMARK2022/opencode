...
export function Prompt(props: PromptProps) {
  ...
  const voiceInput = PromptVoiceInput.createVoiceInputController({
    ...
    transcriber: () =>
      tuiConfig.voice?.transcriber
        ? {
            ...tuiConfig.voice.transcriber,
            args: tuiConfig.voice.transcriber.args ? [...tuiConfig.voice.transcriber.args] : undefined,
          }
        : undefined,
      ...
      const response = await sdk.client.session
        .promptAsync({
          ...
          parts: [
            ...
            ...nonTextParts.map((part) => ({ id: PartID.ascending(), ...part })),
          ],
        })
        .catch((error) => ({ error }))
                ...
                <box flexShrink={0} flexDirection="row" gap={1}>
                  <box marginLeft={1}>
                    <Show when={kv.get("animations_enabled", true)} fallback={<text fg={theme.textMuted}>[⋯]</text>}>
                      <SpinnerIcon color={spinnerDef().color} frames={spinnerDef().frames} interval={40} width={8} />
                    </Show>
                  </box>
