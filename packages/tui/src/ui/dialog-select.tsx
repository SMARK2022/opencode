...
export function DialogSelect<T>(props: DialogSelectProps<T>) {
  ...
  createEffect(
    on(
      () => props.current,
      (current) => {
        const key = currentIdentityKey(current, props.currentKey)
        const currentIndex =
          key === undefined ? flat().findIndex((opt) => isDeepEqual(opt.value, current)) : flat().findIndex((opt) => optionIdentityKey(opt) === key)
        if (currentIndex >= 0) {
  ...
  createEffect(
    on([() => store.filter, () => props.current, () => props.currentKey], ([filter, current, key]) => {
      setTimeout(() => {
        if (filter.length > 0) {
          moveTo(0, true)
        } else {
          const identityKey = currentIdentityKey(current, key)
          const currentIndex =
            identityKey === undefined
              ? flat().findIndex((opt) => isDeepEqual(opt.value, current))
              : flat().findIndex((opt) => optionIdentityKey(opt) === identityKey)
          if (currentIndex >= 0) {
                  ...
                  <For each={options}>
                    {(option) => {
                      ...
                      const current = createMemo(() => {
                        ...
                        if (key === undefined) return isDeepEqual(option.value, props.current)
                        return optionIdentityKey(option) === key
                      })
                      return (
                        <box
