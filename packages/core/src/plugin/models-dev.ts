...
export const ModelsDevPlugin = PluginV2.define({
  ...
  effect: Effect.gen(function* () {
    ...
    const refresh = Effect.fn("ModelsDevPlugin.refresh")(function* () {
      const data = yield* modelsDev.get().pipe(
        Effect.catchCause((cause) =>
          Effect.logWarning("models.dev unavailable; continuing with configured providers only", { cause }).pipe(
            Effect.as({} as Record<string, ModelsDev.Provider>),
          ),
        ),
      )
