...
function permissions(info?: ConfigPermissionV1.Info, tools?: Readonly<Record<string, boolean>>) {
  ...
  for (const [action, rule] of Object.entries(info ?? {})) {
    if (!rule) continue
    if (isPermissionControl(action)) continue
    if (typeof rule === "string") {
      if (isPermissionAction(rule)) rules.push({ action, resource: "*", effect: rule })
      continue
    }
    rules.push(
      ...Object.entries(rule).flatMap(([resource, effect]) =>
        isPermissionAction(effect) ? [{ action, resource, effect }] : [],
      ),
    )
  }
...
function isPermissionControl(action: string) {
  return action === "approvals_reviewer" || action === "auto_review"
}

function isPermissionAction(effect: unknown): effect is ConfigPermissionV1.Action {
  return effect === "allow" || effect === "ask" || effect === "auto" || effect === "deny"
}
