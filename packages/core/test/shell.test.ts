...
describe("shell", () => {
  ...
  if (process.platform === "win32") {
    ...
    test("normalizes /cygdrive paths via windowsPath (does not require Git Bash installed)", () => {
      ...
      expect(FSUtil.windowsPath(input)).toBe("C:/Program Files/Git/bin/bash.exe")
    })

    test("normalizes Git Bash shell paths from env", async () => {
      const gitbash = Shell.gitbash()
      if (!gitbash) return
      const shell = gitbash.replace(/^([A-Za-z]):[\\/](.*)$/u, (_, drive, rest) =>
        `/cygdrive/${drive.toLowerCase()}/${rest.replaceAll("\\", "/")}`,
      )
      await withShell(shell, async () => {
        expect(Shell.preferred()?.replaceAll("\\", "/")).toBe(gitbash.replaceAll("\\", "/"))
      })
    })
