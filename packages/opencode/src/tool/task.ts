...
export const TaskTool = Tool.define(
  ...
  Effect.gen(function* () {
    ...
    const run = Effect.fn("TaskTool.execute")(function* (
      ...
      const parentSessionPermission = Schema.is(PermissionV1.Ruleset)(ctx.extra?.parentSessionPermission)
        ? ctx.extra.parentSessionPermission
        : (parent.permission ?? [])
      ...
      const backgroundRun = runTask().pipe(Effect.provideService(Database.Service, database))

      if (yield* background.extend({ id: nextSession.id, run: backgroundRun })) {
        return {
          title: params.description,
          metadata: {
            ...metadata,
      ...
      const info = yield* background.start({
        ...
        run: backgroundRun.pipe(Effect.onInterrupt(() => ops.cancel(nextSession.id))),
      })
