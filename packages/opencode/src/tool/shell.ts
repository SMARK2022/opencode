...
// Python 专项改写会改变反斜杠和引号含义，已移除；原生 shell 负责解释命令正文。
const ask = Effect.fn("ShellTool.ask")(function* (
  ctx: Tool.Context,
  scan: Scan,
  metadata: { command: string; cwd: string; shell: string },
) {
  // 两个门禁使用同一原命令；源码与stdin角色由共享分析层识别，执行正文始终保持原样。
  if (scan.dirs.size > 0) {
...
function psEncoded(command: string) {
  return Buffer.from(
    [
      ...
      "& {",
      command,
      // 在用户脚本同一作用域观察最终状态，成功恢复优先于此前native程序的残留码。
      "if ($?) { exit 0 }",
      // 失败保留本脚本已有的精确native非零码，纯cmdlet失败使用PowerShell失败码1。
...
// 原 shellGuidance 已合入 ShellPrompt 的对应 shell 分支，版本与引用规则统一生成。
...
export const ShellTool = Tool.define(
  ...
  Effect.gen(function* () {
    ...
    const collect = Effect.fn("ShellTool.collect")(function* (
      ...
      const locals = localCommands(root)
      ...
      if (scan.patterns.size === 0 && locals.length === 0 && parseInput.trim()) {
        scan.patterns.add(parseInput.trim())
      }
    ...
    return () =>
      Effect.gen(function* () {
        ...
        const description = prompt.description + (compressionGuidance ? "\n" + compressionGuidance + "\n" : "")
            ...
            Effect.gen(function* () {
              ...
              yield* Effect.scoped(
                Effect.gen(function* () {
                  ...
                  const scan = yield* collect(tree.rootNode, cwd, ps, shell, instanceCtx, parseInput)
