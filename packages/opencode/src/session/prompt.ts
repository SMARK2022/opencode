...
const elog = {
  info: (message: string, extra?: Record<string, unknown>) => Effect.sync(() => log.info(message, extra)),
  error: (message: string, extra?: Record<string, unknown>) => Effect.sync(() => log.error(message, extra)),
}
...
type PromptReference =
  ...
  | { name: string; kind: "git"; path: string; repository: string; branch?: string }
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const ops = Effect.fn("SessionPrompt.ops")(function* () {
      return {
        cancel: (sessionID: SessionID) => cancel(sessionID).pipe(Effect.orDie) as Effect.Effect<void>,
    ...
    const abortPendingAssistants: (sessionID: SessionID) => Effect.Effect<void> = Effect.fn(
      "SessionPrompt.abortPendingAssistants",
    )(function* (sessionID: SessionID) {
      const pending = [] as (MessageV2.WithParts & { info: MessageV2.Assistant })[]
      const messages = yield* MessageV2.stream(sessionID).pipe(Effect.provideService(Database.Service, database))
      ...
      yield* Effect.forEach(
        ...
        Effect.fnUntraced(function* (msg) {
          ...
          if (requestUsage && assistant.parentID) {
            yield* requestUsage.recordAssistant({ sessionID, requestID: assistant.parentID, assistant })
          }
        }),
        { discard: true },
      )
...
NOTE: At any point in time through this workflow you should feel free to ask the user questions or clarifications. Don't make large assumptions about user intent. The goal is to present a well researched plan to the user, and tie any loose ends before implementation begins.
    ...
    const resolveTools = Effect.fn("SessionPrompt.resolveTools")(function* (input: {
      ...
      for (const item of yield* registry.tools({
        ...
        // [local-smark] ToolSelection 权限过滤
    ...
    const handleSubtask = Effect.fn("SessionPrompt.handleSubtask")(function* (input: {
      ...
      const result = yield* Effect.gen(function* () {
        ...
        return yield* taskTool.execute(taskArgs, {
          ...
          callID: part.callID,
          extra: { bypassAgentCheck: true, parentSessionPermission: session.permission ?? [], promptOps },
          messages: msgs,
    ...
    const shellImpl = Effect.fn("SessionPrompt.shellImpl")(function* (input: ShellInput, ready?: Latch.Latch) {
      return yield* Effect.uninterruptibleMask((restore) =>
        Effect.gen(function* () {
          ...
          const { msg, part, cwd, userMsg } = yield* Effect.gen(function* () {
            const ctx = yield* InstanceState.context
    ...
    const prompt: (input: PromptInput) => Effect.Effect<SessionV1.WithParts, Image.Error> = Effect.fn(
      ...
      if (permissions.length > 0) {
        const toolIDs = new Set(yield* registry.ids())
        const nextPermission = Permission.compact([
          ...(session.permission ?? []).filter((rule) => !toolIDs.has(rule.permission)),
          ...permissions,
        ])
        session.permission = nextPermission
        yield* sessions.setPermission({ sessionID: session.id, permission: nextPermission })
      }

      if (input.noReply === true) {
        // [local-smark] SessionRequestUsage 费用统计完成追踪
    ...
    return Service.of({
      cancel: (sessionID) => cancel(sessionID).pipe(Effect.orDie) as Effect.Effect<void>,
      prompt,
      loop,
      compact,
      shell,
...
const RequestUsageSource = Schema.Literals(["prompt", "command", "shell", "system_compaction", "system_continue", "unknown"])
