...
// Windows launcher 在Bun启动前为两种模式都建立非console pipe；默认drain到Null，
// --print-logs才经wrapper转发。这里的argv||env只决定日志目标和是否保留console关联，
// 不能再承担修复失效stdio HANDLE的责任。
...
// 系统初始化后调用；launcher提供的非console pipe在detach后仍保持有效。
// --print-logs 调试模式保留console及既有Ctrl+C传播，并通过同一组pipe转发日志。
