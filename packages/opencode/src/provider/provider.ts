...
// [local-smark] provider alias/multi-instance support
  ...
  const body = new ReadableStream<Uint8Array>({
    async pull(ctrl) {
      const part = await new Promise<Awaited<ReturnType<typeof reader.read>>>((resolve, reject) => {
        ...
        const id = setTimeout(() => {
          const err = Object.assign(new ProviderError.ResponseStreamError("SSE read timed out"), {
...
function custom(dep: CustomDep): Record<string, CustomLoader> {
  return {
    claudecode: Effect.fnUntraced(function* (provider: Info) {
      ...
      const claudeCodeApiKey = env["CLAUDECODE_API_KEY"]
      const authKey = auth?.type === "api" ? auth.key : undefined
      const apiKey = anthropicAuthToken || claudeCodeApiKey || authKey || provider.options?.apiKey
      const baseURL =
      ...
      const authMode: "bearer" | "x-api-key" = anthropicAuthToken ? "bearer" : "x-api-key"
