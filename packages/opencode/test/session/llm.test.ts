...
function drain(input: LLM.StreamInput): Effect.Effect<void, never, LLM.Service>
function drain(input: LLM.StreamInput, ctx: InstanceContext): Promise<void>
function drain(input: LLM.StreamInput, ctx?: InstanceContext) {
  const effect = LLM.Service.use((svc) => svc.stream(input).pipe(Stream.runDrain))
  if (ctx) return AppRuntime.runPromise(effect.pipe(Effect.provideService(InstanceRef, ctx)))
  return effect
}
...
async function getModel(providerID: ProviderV2.ID, modelID: ModelV2.ID, ctx: InstanceContext) {
  ...
  test("logs provider response and first raw SSE chunk when chunk timeout is enabled", async () => {
    ...
    await withInfoLog(async (logFile) => {
      ...
      await withTestInstance({
        directory: tmp.path,
        fn: async (ctx) => {
          const model = await getModel(ProviderV2.ID.make(providerID), ModelV2.ID.make("gpt-test"), ctx)
  ...
  for (const item of [
    ...
    test(`uses configured provider ${item.name} for request user agent`, async () => {
      ...
      await withTestInstance({
        directory: tmp.path,
        fn: async (ctx) => {
          const resolved = await getModel(ProviderV2.ID.make(providerID), ModelV2.ID.make("gpt-5.5"), ctx)
