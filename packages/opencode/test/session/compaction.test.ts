...
function compactionProcessLayer(options?: CompactionProcessOptions) {
  ...
  const todo = Todo.layer.pipe(Layer.provide(events), Layer.provide(Database.defaultLayer))
  ...
  return Layer.mergeAll(SessionCompaction.layer.pipe(Layer.provide(processor)), processor, events, status, todo).pipe(
