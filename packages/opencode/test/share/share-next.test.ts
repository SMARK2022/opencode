    ...
    provideTmpdirInstance(
      () => {
        ...
        return Effect.gen(function* () {
          ...
          const direct = seen.filter((req) => !req.url.endsWith("/sync"))
          expect(direct).toHaveLength(1)
          expect(direct[0].method).toBe("POST")
          expect(direct[0].url).toBe("https://legacy-share.example.com/api/share")
    ...
    provideTmpdirInstance(
      () => {
        ...
        return Effect.gen(function* () {
          ...
          expect(seen.filter((req) => !req.url.endsWith("/sync")).map((req) => [req.method, req.url])).toEqual([
