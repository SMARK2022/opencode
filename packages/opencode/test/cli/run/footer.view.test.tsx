...
test("direct footer shows editable prompts and additional queued work while running", async () => {
  ...
  try {
    ...
    const spinnerBox = main.getChildren()[0]
    const model = statusItems[2]
    const queued = statusItems[3]
    const hint = statusItems.at(-1)!

    expect(spinnerBox).toBeInstanceOf(BoxRenderable)
    expect((spinnerBox as BoxRenderable).getChildren()[0]).toBeInstanceOf(TextRenderable)
...
test("direct footer shows provider for custom model status", async () => {
  const app = await renderFooter({
    providers: [daxiaoCodexProvider()],
    currentModel: { providerID: "DaXiao Codex", modelID: "gpt-5.5" },
    width: 160,
  })
  ...
  try {
    ...
    expect(frame).toContain("DaXiao Codex")
  } finally {
    app.cleanup()
  }
})
...
test("direct model panel disambiguates custom providers with matching model names", async () => {
  const [providers] = createSignal<RunProvider[] | undefined>([provider(), daxiaoCodexProvider()])
  const [current] = createSignal<RunInput["model"]>({ providerID: "DaXiao Codex", modelID: "gpt-5.5" })
