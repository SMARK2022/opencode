...
这条命令是完整写法：`bash -s --` 表示让 `bash` 从 stdin 读取 installer，并把后面的 `--version 1.15.10-smark` 作为 installer 参数传入。版本参数可以写 `1.15.10-smark`，也可以写 release tag 形式的 `v1.15.10-smark`。
